...
export const VideoRender = ({images=[],
							overlays=[],
							scale=3}) => {
	...
	const ref = useRef('threejs_video');
	...
	useEffect(() => {
		...
		if (frameCount > 0) {
			...
			imageMeta = {"width": images[0] && images[0][0] ? images[0][0].length : 0,
			...
			function animate() {
				...
				if (elapsed > fpsInterval) {
					...
					let image = images[frame.current];
	...
	let buttonStyle = {"height":"40px"};

	return (
		<div>
			<div id="threejs_video" ref={ref}>
			</div>
